...
import { LIFECYCLE_STATUS, VERSION_STATUS, SOCIAL_PLATFORM } from '@/lib/admin/constants/enums';
...
/**
 * Fetch every ACTIVE, non-deleted, currently-PUBLISHED talent from
 * Postgres, mapped to the public shape. Returns `null` (not `[]`) when the
 * database isn't configured or the read fails, so callers can tell "no DB"
 * apart from "DB has zero published talents" — both fall back to static
 * data, but the distinction is useful for logging.
 *
...
const fetchPublishedTalents = cache(async function fetchPublishedTalents() {
  ...
  try {
    ...
    return talents
      .map(mapPublishedTalentToPublicShape)
      .filter((t) => t !== null);
  } catch (error) {
...
/**
 * Resolves which talent list the public site should render: the live
 * database list if it loaded successfully and has at least one published
 * talent, otherwise the static file. Every exported function below is
 * built on top of this single decision so the roster and detail pages
 * never disagree about which source is "live" for a given request.
 *
 * @returns {Promise<{ source: 'db' | 'static', talents: object[] }>}
 */
const resolveTalentSource = cache(async function resolveTalentSource() {
  const dbTalents = await fetchPublishedTalents();
  if (dbTalents && dbTalents.length > 0) {
    return { source: 'db', talents: dbTalents };
  }
  return { source: 'static', talents: staticTalentList };
});

/**
 * Full talent list for the public site, sorted the same way
 * app/[locale]/talent/page.jsx already sorts the static list
 * (`sortOrder` ascending). Mirrors `data/talent/index.js`'s exported
 * `talentList`, just resolved from DB-or-static.
 *
 * @returns {Promise<object[]>}
 */
export async function getPublicTalentList() {
  const { talents } = await resolveTalentSource();
  return [...talents].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
}
