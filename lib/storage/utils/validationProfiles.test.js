...
describe('getValidationProfile', () => {
  ...
  // Profile Image Upload Fix sprint (2026-08-25) — profile's cap was
  // lowered to 4MB (below the deployment platform's ~4.5MB request-body
  // limit, per production evidence: a 12.8MB upload failed with
  // FUNCTION_PAYLOAD_TOO_LARGE). Gallery and podcast are asserted
  // separately below to stay at 8MB — this is a profile-only change.
  it('returns the profile-photo profile with the same allowlist and a 4MB cap', () => {
    const profile = getValidationProfile('profile');
    expect(profile.allowedMimeTypes).toEqual(['image/jpeg', 'image/png', 'image/webp']);
    expect(profile.maxBytes).toBe(4 * 1024 * 1024);
  });

  it('does not affect gallery or podcast caps — both stay at 8MB', () => {
    expect(getValidationProfile('gallery').maxBytes).toBe(8 * 1024 * 1024);
    expect(getValidationProfile('podcast').maxBytes).toBe(8 * 1024 * 1024);
  });
