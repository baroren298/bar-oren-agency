 ...
 *
 * Sprint 5a (Recent Publishes) extends getOwnerDashboard() with a fifth,
 * bounded section: the most recently published talent details/socials/
 * gallery changes, newest first, grouped the same way as the other three
 * sections (per talent + work type). See publishTimestamp()'s docstring
 * below for the one load-bearing assumption this section rests on
 * (approvedAt as a publish-time proxy) — it is deliberately documented in
 * code, not just here, since it's the kind of thing that quietly stops
 * being true if a decoupled publish step is ever introduced.
 *
...
import { adminTalentPath } from '../talent-route';
...
/**
 * Deep link for a queue item — see header note on why it's tab-less for
 * now. Clean Admin Talent URL sprint: prefers the parent Talent's current
 * published slug (every repository row here selects `talent.slug`); the
 * internal id is only a fallback for a row that somehow lacks one — and an
 * id-based link still works via the workspace page's ID→slug redirect.
 */
function buildHref(talentId, talentSlug) {
  return adminTalentPath({ id: talentId, slug: talentSlug ?? null });
}
...
function groupRowsByTalent(rows, workType) {
  ...
  return [...byTalent.entries()].map(([talentId, group]) => {
    ...
    return {
      ...
      href: buildHref(talentId, earliest.talent?.slug),
    };
  });
}

/**
 * Sprint 5a (Recent Publishes) — a published row's "when".
 *
 * ASSUMPTION, valid only while approval and publish remain coupled: this
 * engine has no separate "publish" action or timestamp today — approving a
 * proposal IS publishing it (see publishService.publish() /
 * talentRepository.publishTalentVersion, which set approvedAt/approvedById
 * and flip status to PUBLISHED in the same transaction). approvedAt is
 * therefore used here as the publish-time proxy. The updatedAt/createdAt
 * fallback exists for historical rows that predate consistent approvedAt
 * bookkeeping (TalentVersion has no updatedAt column at all, so its
 * fallback chain ends at createdAt). If a decoupled publish step is ever
 * introduced, this function — and the RecentPublishItem.publishedAt it
 * feeds — must be repointed at that event's own timestamp, not approvedAt.
 *
 * @param {{ approvedAt: Date|null, updatedAt?: Date, createdAt: Date }} row
 * @returns {Date}
 */
function publishTimestamp(row) {
  return row.approvedAt ?? row.updatedAt ?? row.createdAt;
}

/**
 * Fold per-row published details/socials/gallery rows into one
 * RecentPublishItem per (talent, work type) — same grouping convention as
 * groupRowsByTalent() above, but keyed on publishTimestamp() instead of
 * rowTimestamp(), and reporting the LATEST (not earliest) touch, since this
 * section asks "what shipped most recently for this talent?" rather than
 * "how long has this been waiting?". For DETAILS, only one TalentVersion
 * per talent can be PUBLISHED at a time (publishing supersedes the prior
 * one), so grouping is a safety net there rather than the common case; for
 * SOCIALS/GALLERY, several rows legitimately publish together and grouping
 * is the normal path, exactly as in the other three sections.
 *
 * publishedBy comes from the latest row's approvedBy — the person who
 * actually approved/published it — and is null whenever that column is
 * unset (historical rows, or any row an Owner-bypass path left unattributed);
 * callers must render that as "—", never assume an actor is present.
 *
 * @param {Array} rows - dashboardRepository recent-published rows
 * @param {string} workType - DASHBOARD_WORK_TYPE value
 * @returns {Array} RecentPublishItem[] (see dashboardDto typedef)
 */
function groupPublishedRowsByTalent(rows, workType) {
  const byTalent = new Map();
  for (const row of rows) {
    if (!byTalent.has(row.talentId)) byTalent.set(row.talentId, []);
    byTalent.get(row.talentId).push(row);
  ...
  return [...byTalent.entries()].map(([talentId, group]) => {
    const sorted = [...group].sort((a, b) => publishTimestamp(a) - publishTimestamp(b));
    const latest = sorted[sorted.length - 1];
    return {
      key: `${workType}:${talentId}`,
      workType,
      talentId,
      talentName: latest.name ?? latest.talent?.currentPublishedVersion?.name ?? latest.talent?.slug ?? null,
      itemCount: group.length,
      publishedBy: toActor(latest.approvedBy),
      publishedAt: publishTimestamp(latest).toISOString(),
      href: buildHref(talentId, latest.talent?.slug),
    };
  });
}
...
function toDetailsItem(version) {
  return {
    ...
    href: buildHref(version.talentId, version.talent?.slug),
  };
}
...
export const dashboardService = {
  ...
  async getOwnerDashboard({ actorId, actorRole } = {}) {
    ...
    const [
      ...
      draftSocials,
      draftGallery,
      recentPublishedVersions,
      recentPublishedSocials,
      recentPublishedGallery,
    ] = await Promise.all([
      ...
      // Sprint 5a (Recent Publishes) — added to this same parallel batch,
      // not a follow-up round-trip (Section 13.15: bounded, no N+1).
      dashboardRepository.listRecentPublishedTalentVersions(),
      dashboardRepository.listRecentPublishedTalentSocials(),
      dashboardRepository.listRecentPublishedTalentGalleryImages(),
    ]);
    ...
    // ── Recent Publishes (Sprint 5a) ──────────────────────────────────────
    // See publishTimestamp()'s docstring: publishedAt is approvedAt used as
    // a publish-time proxy, valid only while approval and publish remain
    // coupled in this engine.
    const recentPublishes = [
      ...groupPublishedRowsByTalent(recentPublishedVersions, DASHBOARD_WORK_TYPE.DETAILS),
      ...groupPublishedRowsByTalent(recentPublishedSocials, DASHBOARD_WORK_TYPE.SOCIALS),
      ...groupPublishedRowsByTalent(recentPublishedGallery, DASHBOARD_WORK_TYPE.GALLERY),
