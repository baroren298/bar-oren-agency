...
describe('dashboardService.getMyWork — UI contract', () => {
  it('carries the talent fields, hrefs, and serializability the page needs', async () => {
    ...
    expect(socials).toMatchObject({
      ...
      href: '/admin/talent/new-slug',
    });

    // Every item deep-links to the talent workspace by slug and survives
    // the server/client JSON boundary unchanged.
    for (const item of items) {
      expect(item.href).toMatch(/^\/admin\/talent\/[^/]+$/);
      expect(item.href).not.toContain(item.talentId);
    }
