...
describe('safe projection', () => {
  ...
  it('Sprint 3b — allowlists scope for a single session revocation and drops everything else', async () => {
    hoisted.listRecent.mockResolvedValue([
      row({
        actionType: 'SESSION_REVOKED',
        updatedBy: OWNER,
        metadataAfter: { scope: 'single', sid: 'should-never-appear', someOtherField: 'x' },
      }),
    ]);

    const { entries } = await auditLogService.listEntries({ actorRole: 'OWNER' });
    expect(entries[0].details).toEqual({ scope: 'single' });
    expect(JSON.stringify(entries)).not.toContain('should-never-appear');
  });

  it('Sprint 3b — allowlists scope + revokedCount for a revoke-all', async () => {
    hoisted.listRecent.mockResolvedValue([
      row({
        actionType: 'SESSIONS_REVOKED',
        updatedBy: OWNER,
        metadataAfter: { scope: 'all', revokedCount: 3 },
      }),
    ]);

    const { entries } = await auditLogService.listEntries({ actorRole: 'OWNER' });
    expect(entries[0].details).toEqual({ scope: 'all', revokedCount: 3 });
  });

  it('Sprint 3b — rejects a non-numeric or invalid scope value rather than passing it through', async () => {
    hoisted.listRecent.mockResolvedValue([
      row({
        actionType: 'SESSIONS_REVOKED',
        updatedBy: OWNER,
        metadataAfter: { scope: 'everything', revokedCount: 'three' },
      }),
    ]);
