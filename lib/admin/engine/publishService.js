...
import { revalidatePublicTalentPages } from '../cache/revalidatePublicTalentPages';
import { eventService } from './eventService';
import { EVENT_TYPE } from './eventTypes';
import { VERSION_STATUS, REVISION_CONFLICT_ERROR_CODE, ROLE } from '../constants/enums';

/**
 * Defense in depth (OWNER/EMPLOYEE Permission Model Sprint): publish() is
 * the only code path that can ever set a version PUBLISHED (see header
 * comment above), so it must not trust that every caller already checked
 * the actor's role — it verifies independently, the same way
 * approvalService does for approve/reject, rather than relying solely on
 * route protection or on approvalService's own check.
 */
function assertActorIsOwner(actorRole) {
  if (actorRole !== ROLE.OWNER) {
    const err = new Error(
      `[publishService.publish] actorRole "${actorRole}" is not permitted — only OWNER may publish.`
    );
    err.statusCode = 403;
...
export const publishService = {
   ...
   * @param {string} params.actorRole - must be ROLE.OWNER (defense in
   *   depth — see assertActorIsOwner above); throws a 403-shaped error
   *   otherwise.
  ...
  async publish(
    adapter,
    { parentId, versionId, actorId, actorRole, basedOnRevisionNumber, correlationId } = {}
  ) {
    ...
    // Invalidate the public talent ISR pages so a publish is visible
    // immediately instead of waiting on their time-based `revalidate`
    // window (see app/[locale]/talent/page.jsx and
    // app/[locale]/talent/[slug]/page.jsx, both `revalidate = 60`).
    // Delegated to lib/admin/cache/revalidatePublicTalentPages.js (keeps
    // `next/cache` out of this generic, entity-agnostic service and out of
    // its Vitest-run unit tests — see that file's header for why).
    revalidatePublicTalentPages();
