 ...
 * Management / Roles / Permissions audit: OWNER may edit/submit/discard any
 * TalentVersion draft; EMPLOYEE may only do so for a DRAFT/PROPOSED version
 * they themselves created (`version.createdById === actorId`). This is the
 * actual server-side enforcement — any client-side hiding of Save/Submit/
 * Cancel Editing controls for another author's draft is just a courtesy on
 * top of this, same relationship this file's other status guards (e.g.
 * "only a DRAFT can be discarded") already have to their own route-level
 * pre-checks.
 *
 * Deliberately local/duplicated (like assertActorIsOwner in
 * galleryService.js/socialsService.js) rather than a shared capability
 * helper — out of scope for this sprint per its own instructions ("do not
 * introduce capability helper yet").
 *
 * @param {string} actorRole
 * @param {string} actorId
 * @param {object} version - must have .id and .createdById
 * @param {string} action - calling method name, for the error message
 */
function assertActorCanModifyVersion(actorRole, actorId, version, action) {
  if (actorRole === ROLE.OWNER) return;
  if (version.createdById && version.createdById === actorId) return;
  const err = new Error(
    `[${action}] actor "${actorId}" (role "${actorRole}") may not modify version ` +
      `"${version.id}" — it was created by a different user, and only its author or an ` +
      'OWNER may modify it.'
  );
  err.statusCode = 403;
...
export const proposalService = {
   ...
   *   submit any DRAFT; EMPLOYEE only one they created themselves.
   * @param {string} [params.correlationId]
   * @returns {Promise<object>} the updated version row
   */
  async submit(adapter, { parentId, versionId, actorId, actorRole, correlationId } = {}) {
    assertImplementsAdapterContract(adapter);
    ...
    if (!actorId) {
      throw new Error('[proposalService.submit] actorId is required.');
    }
    ...
    assertActorCanModifyVersion(actorRole, actorId, version, 'proposalService.submit');
   ...
   * @param {string} params.actorId
   * @param {string} [params.actorRole] - Draft Ownership Sprint 1: OWNER may
   *   edit any DRAFT/PROPOSED version; EMPLOYEE only one they created themselves.
  ...
  async update(
    adapter,
    { parentId, versionId, fields, actorId, actorRole, basedOnRevisionNumber, correlationId } = {}
  ) {
    assertImplementsAdapterContract(adapter);
    ...
    // Draft Ownership Sprint 1 — same server-side authority as the status
    // guard just above: OWNER may edit any draft, EMPLOYEE only their own.
    assertActorCanModifyVersion(actorRole, actorId, version, 'proposalService.update');
  ...
  /**
   * Cancel Editing / Discard Draft sprint — abandon a DRAFT proposal
   * outright and return to the Published version (which is untouched by
   * this call; it's simply a different row that already exists). Mirrors
   * `submit()`'s "load, check status, act" shape, with two differences:
   *
   *   1. DRAFT-only (not DRAFT-or-PROPOSED like `update()`): a PROPOSED
   *      version is mid-review and is explicitly out of scope for this
   *      action per this sprint's business rule — withdrawing/cancelling a
   *      submitted proposal remains exclusively the Owner Reject flow
   *      (`approvalService.reject()`), which this method never calls and
   *      does not change.
   *   2. Calls `adapter.discardVersion(versionId)` directly (not part of
   *      `assertImplementsAdapterContract`'s required shape), same optional-
   *      capability pattern `update()` already uses for
   *      `updateProposedVersion` — a clear error is thrown if the adapter
   *      doesn't implement it, rather than a confusing "not a function"
   *      crash.
   *
   * Emits `ProposalDiscarded` (catalogued in eventTypes.js, mapped to
   * ActionType.DELETED in auditLogListener).
   *
   * @param {object} adapter - Section 13.10 contract, plus the optional
   *   `discardVersion(versionId)` capability
   * @param {object} params
   * @param {string} params.parentId
   * @param {string} params.versionId
   * @param {string} params.actorId
   * @param {string} [params.actorRole] - Draft Ownership Sprint 1: OWNER may
   *   discard any DRAFT; EMPLOYEE only one they created themselves.
   * @param {string} [params.correlationId]
   * @returns {Promise<{ discarded: true }>}
   */
  async discard(adapter, { parentId, versionId, actorId, actorRole, correlationId } = {}) {
    assertImplementsAdapterContract(adapter);

    if (typeof adapter.discardVersion !== 'function') {
      throw new Error(
        `[proposalService.discard] adapter "${adapter.entityType}" does not implement ` +
          'discardVersion — Cancel Editing is not supported for this entity type yet.'
      );
    }
    if (!parentId) {
      throw new Error('[proposalService.discard] parentId is required.');
    }
    if (!versionId) {
      throw new Error('[proposalService.discard] versionId is required.');
    }
    if (!actorId) {
      throw new Error('[proposalService.discard] actorId is required.');
    }

    const version = await adapter.getVersion(versionId);
    if (!version) {
      throw new Error(`[proposalService.discard] no version found for id "${versionId}".`);
    }
    // Server-side authority (this sprint's locked business rule) — the
    // actual enforcement; any client-side hiding of the Cancel Editing
    // button is just a courtesy on top of this.
    if (version.status !== VERSION_STATUS.DRAFT) {
      throw new Error(
        `[proposalService.discard] version "${versionId}" is "${version.status}", not DRAFT — ` +
          'only a DRAFT proposal can be discarded this way (a PROPOSED version can only be ' +
          'withdrawn via Owner Reject).'
      );
    }
    // Draft Ownership Sprint 1 — OWNER may discard any draft, EMPLOYEE only
    // their own (previously: "both OWNER and EMPLOYEE may discard a Draft,
    // no extra role check" — superseded by the permission-model audit's
    // locked decision).
    assertActorCanModifyVersion(actorRole, actorId, version, 'proposalService.discard');

    await adapter.discardVersion(versionId);

    await eventService.emit(EVENT_TYPE.PROPOSAL_DISCARDED, {
      entityType: adapter.entityType,
      entityId: parentId,
      actorId,
      correlationId,
