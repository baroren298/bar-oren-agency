...
export const proposalService = {
   ...
   * place (locked decision: edits update the DRAFT row, never create a new
   * version). Mirrors `submit()`'s "load, check status, act" shape above,
   * with four differences required by this feature's explicit scope:
   *
   *   1. DRAFT-only editability (required safeguard #4): any other status
   *      (PROPOSED/PUBLISHED/REJECTED/SUPERSEDED) throws. This is the
  ...
  async update(
    ...
    // Server-side authority (required safeguard #4) — DRAFT is the only
    // editable status. This is the actual enforcement; any client-side
    // disabling of the Save Draft button is just a courtesy on top of this.
    if (version.status !== VERSION_STATUS.DRAFT) {
      throw new Error(
        `[proposalService.update] version "${versionId}" is "${version.status}", not DRAFT — ` +
