...
export const EVENT_TYPE = Object.freeze({
  ...
  // events, emitted by lib/admin/clientService.js after each committed
  // mutation. Entity-prefixed names per this file's convention note:
  // Clients/Brands are internal operational records with no
  // proposal/version lifecycle (same rationale as the User prefix above),
  // so the generic Proposal* vocabulary doesn't apply. The Event row's
  // entityType is CLIENT/BRAND and entityId is the client/brand id; the
  // acting user goes in actorId. Payloads are allowlisted at emission —
  // entity name, status, and for updates the changed field NAMES only;
  // never contact email/phone/notes values (no personal data in
  // Event/AuditLog).
  ...
  // Talent Archive & Restore feature — emitted by
  // lib/admin/talentArchiveService.js after each committed status
  // transition. Entity-prefixed per this file's convention note: Talent's
  // entity-level lifecycle (archive/restore) is a different axis from the
  // generic Proposal* vocabulary above (which covers TalentVersion's
  // content lifecycle, not the parent Talent row's own status), and has no
  // generic cross-entity equivalent yet — same rationale as the Client/
  // Brand prefix above. The Event row's entityType is TALENT and entityId
  // is the talent id; the acting Owner goes in actorId. TalentRestored is
  // this codebase's first real use of ACTION_TYPE.RESTORED (see
  // auditLogListener.js).
  TALENT_ARCHIVED: 'TalentArchived',
  TALENT_RESTORED: 'TalentRestored',
});
