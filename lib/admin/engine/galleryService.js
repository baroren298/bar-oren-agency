 ...
 * changes needed.
 *
 * ONE DELIBERATE DIFFERENCE FROM socialsService.saveDraft(): TalentSocial
 * supports a "no id -> insert a brand-new DRAFT with basedOnVersionId:
 * null" case, because an employee can add a brand-new social account from
 * nothing. Gallery Sprint 1 has no equivalent — "Add Image"/"Replace
 * Image"/"Upload" are explicitly out of scope this sprint (no storage
 * provider is implemented, see ADMIN_PANEL_PLAN.md Gallery Sprint 1 scope).
 * Every TalentGalleryImage row's `imageAssetId` is a required FK; this
 * sprint only versions metadata (order/altHe/altEn/position/scale/
 * mobileOrder) for images that already exist in the gallery. So
 * saveDraft() below REQUIRES `id` on every entry and throws a recognizable
 * `MISSING_IMAGE_ID` error otherwise, rather than silently doing nothing or
 * (worse) trying to invent a row with no backing asset.
 *
...
export const galleryService = {
   ...
   */
  async approve(adapter, { parentId, imageId, actorId } = {}) {
    if (!parentId) {
   ...
   * @returns {Promise<{ image: object }>}
   */
  async reject(adapter, { parentId, imageId, actorId, rejectionNote } = {}) {
    if (!parentId) {
