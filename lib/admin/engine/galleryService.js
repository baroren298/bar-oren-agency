...
/**
 * Auth Hardening + Draft Ownership Sprint 1 — decision from the User
 * Management / Roles / Permissions audit: OWNER may modify any
 * TalentGalleryImage draft; EMPLOYEE may only modify a DRAFT/PROPOSED row
 * they themselves created (`row.createdById === actorId`). This is the
 * actual server-side enforcement — any client-side hiding of edit controls
 * for another author's row is just a courtesy on top of this. Mirrors
 * socialsService.js's identically-named/identically-shaped helper; note
 * TalentGalleryImage.createdById is nullable for pre-existing historical
 * rows (see prisma/schema.prisma's doc comment) — a null createdById never
 * matches actorId, so a legacy row with no known author is treated the same
 * as "not yours" for a non-OWNER actor (fails closed, not open). Deliberately
 * local/duplicated rather than a shared capability helper (out of scope
 * this sprint).
 *
 * @param {string} actorRole
 * @param {string} actorId
 * @param {object} row - must have .id and .createdById
 * @param {string} action - calling method name, for the error message
 */
function assertActorCanModifyDraft(actorRole, actorId, row, action) {
  if (actorRole === ROLE.OWNER) return;
  if (row.createdById && row.createdById === actorId) return;
  const err = new Error(
    `[${action}] actor "${actorId}" (role "${actorRole}") may not modify gallery image ` +
      `"${row.id}" — it was created by a different user, and only its author or an OWNER may ` +
      'modify it.'
  );
  err.statusCode = 403;
  err.code = 'FORBIDDEN_NOT_DRAFT_OWNER';
  throw err;
}
...
export const galleryService = {
   ...
   *
   * Gallery Image Removal sprint — `lifecycleStatus` is now one of the
   * forwarded fields (see the `fields` object below), which is the entire
   * mechanism for removal. No new branch was needed:
   *   - Removing a PUBLISHED image goes through branch 1 above: the clone
   *     is inserted with `lifecycleStatus: 'HIDDEN'` instead of the ACTIVE
   *     default, `basedOnVersionId` pointing at the still-live row. The
   *     live row itself is untouched by this call — it only stops being
   *     public once that HIDDEN draft is Submitted and Approved (unchanged
   *     approve/publish/supersede logic in talentRepository).
   *   - Removing/withdrawing a DRAFT or PROPOSED image that has no live
   *     PUBLISHED counterpart (`basedOnVersionId` null, or pointing at a
   *     row that's no longer PUBLISHED) goes through branch 2 above: the
   *     SAME row is marked `lifecycleStatus: 'HIDDEN'` in place. Nothing is
   *     ever public under that identity, so this needs no Submit/Approve to
   *     "take effect" — it already stops appearing in
   *     getDraftOrProposedGalleryImagesForTalent (which filters
   *     lifecycleStatus: ACTIVE) the moment this call returns. Whether it
   *     also still shows in Owner Review if it happens to be PROPOSED is
   *     gallery-review.js's decision (buildGalleryReviewItems excludes a
   *     hidden row with no matched published counterpart) — not this
   *     function's.
   *
   ...
   * @param {string} [params.actorRole] - Draft Ownership Sprint 1: required to
   *   edit an existing DRAFT/PROPOSED row in place unless it's the actor's own
   *   (OWNER may edit any; the PUBLISHED-clone and brand-new-insert branches
   *   below are unaffected — a caller always owns their own new draft).
   * @returns {Promise<{ images: object[] }>}
   */
  async saveDraft(adapter, { parentId, images, actorId, actorRole } = {}) {
    if (!parentId) {
    ...
    for (const image of images) {
      const fields = {
        ...
        // Gallery Image Removal sprint — forwarded as-is. Undefined (a
        // normal metadata edit that doesn't touch removal) is a no-op on
        // both write paths below: insertDraftGalleryImage defaults it to
        // ACTIVE, and Prisma's update() ignores an explicit `undefined` in
        // `data` rather than writing it.
        lifecycleStatus: image.lifecycleStatus,
      };
      ...
      if (existing.versionStatus === VERSION_STATUS.PUBLISHED) {
        ...
        // Draft Ownership Sprint 1 — editing an existing Draft/Proposed row
        // in place (unlike the PUBLISHED branch above, which always inserts
        // a brand-new row the actor owns): OWNER may edit any row, EMPLOYEE
        // only one they created themselves.
        assertActorCanModifyDraft(actorRole, actorId, existing, 'galleryService.saveDraft');
   ...
   *
   * Pre-merge blocker fix sprint (QA finding #4) — optional
   * `params.createdById`, mirroring socialsService.submit(): when set, only
   * DRAFT rows authored by that user are flipped. Used by the Owner
   * direct-publish route; the plain Submit route passes nothing — unchanged
   * behavior for an OWNER actor.
   *
   * Draft Ownership Sprint 1 — mirrors socialsService.submit()'s scoping
   * fix exactly: this bulk action has no per-row id for a caller to target,
   * so "EMPLOYEE may only submit drafts they created" is enforced by
   * scoping, not by throwing — whenever `actorRole` isn't OWNER, the
   * effective `createdById` filter is forced to `actorId` regardless of what
   * was passed in. An OWNER's `createdById` argument (or lack of one) is
   ...
   * @param {string} params.parentId - the Talent id
   * @param {string} params.actorId
   * @param {string} [params.actorRole] - non-OWNER forces the scope to `actorId`
   * @param {string} [params.createdById] - only submit DRAFT rows created by this
   *   user; ignored for a non-OWNER actor (always scoped to their own actorId instead)
   * @returns {Promise<{ images: object[] }>}
   */
  async submit(adapter, { parentId, actorId, actorRole, createdById } = {}) {
    if (!parentId) {
    ...
    const scopedCreatedById = actorRole === ROLE.OWNER ? createdById : actorId;
    const submitted = await adapter.submitDraftGalleryImages(parentId, { createdById: scopedCreatedById });
   ...
   * @param {string} params.actorId
   * @param {string} [params.actorRole] - Draft Ownership Sprint 1: OWNER may
   *   resume any REJECTED row; EMPLOYEE only one they themselves authored.
   *   A legacy row with a null createdById (pre-dates this column, see
   *   prisma/schema.prisma) is never treated as "yours" for a non-OWNER
   *   actor — see assertActorCanModifyDraft's header comment.
   * @returns {Promise<{ image: object }>}
   */
  async resumeRejected(adapter, { parentId, imageId, actorId, actorRole } = {}) {
    if (!parentId) {
    ...
    assertActorCanModifyDraft(actorRole, actorId, rejected, 'galleryService.resumeRejected');
