...
export const talentAdapter = {
  ...
  /**
   * Cancel Editing / Discard Draft sprint — pure translation to the
   * repository's delete primitive. Same optional-method pattern as
   * `updateProposedVersion` above (not part of adapterContract.js's
   * REQUIRED_ADAPTER_METHODS — only proposalService.discard() calls this,
   * and only for the Talent entity so far per this sprint's stated scope).
   * No status check here — that's proposalService.discard()'s job, same
   * split `submitVersion`/`rejectVersion` already follow.
   *
   * @param {string} versionId
   */
  async discardVersion(versionId) {
    return talentRepository.deleteTalentVersion(versionId);
  },
  ...
  /**
   * Talent Archive & Restore — pure translation to the repository's status
   * transition primitive. No decision made here (Section 13.16): the
   * OWNER-only check and the idempotence guard (already archived?) live in
   * talentArchiveService, exactly like every other decision/adapter split
   * in this file.
   *
   * @param {string} parentId - the Talent id
   * @param {object} meta
   ...
   */
  async archiveParent(parentId, { actorId } = {}) {
    return talentRepository.archiveTalent(parentId, actorId);
  },

  /**
   * Talent Archive & Restore — pure translation to the repository's
   * inverse status transition primitive. Same optional-method pattern as
   * `updateProposedVersion` above (not part of adapterContract.js's
   * REQUIRED_ADAPTER_METHODS — only talentArchiveService calls this).
   *
   * @param {string} parentId - the Talent id
   */
  async restoreParent(parentId) {
    return talentRepository.restoreTalent(parentId);
  },
   ...
   *
   * Pre-merge blocker fix sprint (QA finding #4) — optional `meta.createdById`
   * forwarded to the repository so the Owner direct-publish route can limit
   * the flip to the acting user's own drafts. Still pure translation; the
   * scoping decision belongs to the caller.
   *
   * @param {string} parentId - the Talent id
   * @param {object} [meta]
  ...
  async submitDraftSocials(parentId, { createdById } = {}) {
    return talentRepository.submitDraftSocialsForTalent(parentId, { createdById });
  },
   ...
   * talent. Sibling to `submitDraftSocials`, including the Pre-merge
   * blocker fix sprint's optional `meta.createdById` scoping (see that
  ...
  async submitDraftGalleryImages(parentId, { createdById } = {}) {
    return talentRepository.submitDraftGalleryImagesForTalent(parentId, { createdById });
  },
