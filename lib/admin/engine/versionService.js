...
export const versionService = {
  ...
  /**
   * List every parent row of one content type — Sprint 4.1, added for the
   * first read-only Presentation-layer page (`/admin/talent`, Section 2).
   * Pure pass-through to `adapter.listParents()`: no entity-specific
   * branching, no shape decision of its own (Section 13.9/13.16) — this
   * exists only because Section 13.15 forbids Presentation from skipping
   * the engine to call a repository's list query directly, even for a
   * trivial, decision-free read.
   *
   * @param {object} adapter - Section 13.10 contract
   * @param {object} [opts]
   ...
   * @returns {Promise<object[]>}
   */
  async listParents(adapter, opts) {
    assertImplementsAdapterContract(adapter);
    const parents = await adapter.listParents(opts);
