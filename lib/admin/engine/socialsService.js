...
export const socialsService = {
   ...
   * @param {string} params.parentId - the Talent id
   * @param {string} params.actorId
   * @param {string} [params.createdById] - only submit DRAFT rows created by this user
   * @returns {Promise<{ accounts: object[] }>}
   */
  async submit(adapter, { parentId, actorId, createdById } = {}) {
    if (!parentId) {
    ...
    const submitted = await adapter.submitDraftSocials(parentId, { createdById });
