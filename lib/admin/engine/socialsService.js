 ...
 * Management / Roles / Permissions audit: OWNER may modify any TalentSocial
 * draft; EMPLOYEE may only modify a DRAFT/PROPOSED row they themselves
 * created (`row.createdById === actorId`). This is the actual server-side
 * enforcement — any client-side hiding of edit controls for another
 * author's row is just a courtesy on top of this. Mirrors
 * proposalService.js's identically-named/identically-shaped helper for
 * TalentVersion; deliberately local/duplicated rather than a shared
 * capability helper (out of scope this sprint).
 *
...
function assertActorCanModifyDraft(actorRole, actorId, row, action) {
  ...
  const err = new Error(
    `[${action}] actor "${actorId}" (role "${actorRole}") may not modify social account ` +
...
export const socialsService = {
   ...
   */
  async saveDraft(adapter, { parentId, accounts, actorId, actorRole } = {}) {
    if (!parentId) {
    ...
    for (const account of accounts) {
      ...
      if (account.id) {
        ...
        if (existing.versionStatus === VERSION_STATUS.PUBLISHED) {
          ...
          // Draft Ownership Sprint 1 — editing an existing Draft/Proposed
          // row in place (unlike the PUBLISHED branch above, which always
          // inserts a brand-new row the actor owns): OWNER may edit any row,
          // EMPLOYEE only one they created themselves.
          assertActorCanModifyDraft(actorRole, actorId, existing, 'socialsService.saveDraft');
   ...
   * `params.createdById`: when set, only DRAFT rows authored by that user
   * are flipped (forwarded through adapter.submitDraftSocials to the
   * repository's WHERE clause). The Owner direct-publish route passes the
   * acting Owner's id so another author's unfinished drafts are never
   * submitted behind their back; the plain Submit route passes nothing —
   * unchanged behavior for an OWNER actor.
   *
   * Draft Ownership Sprint 1 — this bulk action has no per-row id for a
   * caller to target, so "EMPLOYEE may only submit drafts they created"
   * (the audit's locked decision) is enforced by *scoping*, not by throwing:
   * whenever `actorRole` isn't OWNER, the effective `createdById` filter is
   * forced to `actorId` regardless of what was passed in, so an EMPLOYEE's
   * plain Submit click only ever flips their own DRAFT rows — another
   * author's (Employee or Owner) unfinished drafts are silently left alone,
   * the same "untouched, not swept up" outcome the pre-merge fix already
   * gave the Owner publish shortcut, now guaranteed for every actor. An
   * OWNER's `createdById` argument (or lack of one) is passed through
   * unchanged — this preserves "OWNER may submit any draft."
   *
  ...
  async submit(adapter, { parentId, actorId, actorRole, createdById } = {}) {
    ...
    const submitted = await adapter.submitDraftSocials(parentId, { createdById: scopedCreatedById });
   ...
   *   this route's own doc comment already describes resuming as "the editor
   *   fixing their own work," this is that rule's actual enforcement.
   * @returns {Promise<{ account: object }>}
   */
  async resumeRejected(adapter, { parentId, socialId, actorId, actorRole } = {}) {
    if (!parentId) {
    ...
    assertActorCanModifyDraft(actorRole, actorId, rejected, 'socialsService.resumeRejected');
