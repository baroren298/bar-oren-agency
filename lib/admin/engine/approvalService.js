...
/**
 * Defense in depth (OWNER/EMPLOYEE Permission Model Sprint): route-level
 * role gating (requireOwner) is the first line of defense, but approve/
 * reject must never depend on route protection alone — the engine itself
 * refuses to act for any actor whose role isn't OWNER. Throws a 403-shaped
 * error so callers (route handlers) can map it straight to an HTTP status
 * the same way they already map requireOwner()'s error.
 */
...
export const approvalService = {
  ...
  async approve(
    ...
    assertActorIsOwner(actorRole, 'approvalService.approve');
  ...
  async reject(
    adapter,
    { parentId, versionId, actorId, actorRole, rejectionNote, correlationId } = {}
  ) {
    assertImplementsAdapterContract(adapter);
    ...
    assertActorIsOwner(actorRole, 'approvalService.reject');
