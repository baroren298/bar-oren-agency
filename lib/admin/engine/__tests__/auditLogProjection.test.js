...
describe('AuditLog as a projection of the Event stream (real auditLogListener, fake repositories)', () => {
  ...
  describe('user-management event projections (Sprint 2a — mapping only, no emitters yet)', () => {
    ...
    it.each([
      ...
      // Sprint 3b (Session Management API).
      [EVENT_TYPE.USER_SESSION_REVOKED, ACTION_TYPE.SESSION_REVOKED, 'updatedById'],
      [EVENT_TYPE.USER_SESSIONS_REVOKED, ACTION_TYPE.SESSIONS_REVOKED, 'updatedById'],
    ])(
    ...
    it('Sprint 3b — stores the session-revocation payload ({ scope } / { scope, revokedCount }) as metadataAfter', async () => {
      await emitUserEvent(EVENT_TYPE.USER_SESSION_REVOKED, { payload: { scope: 'single' } });
      await emitUserEvent(EVENT_TYPE.USER_SESSIONS_REVOKED, { payload: { scope: 'all', revokedCount: 3 } });

      expect(hoisted.auditLogRows).toHaveLength(2);
      expect(hoisted.auditLogRows[0].metadataAfter).toEqual({ scope: 'single' });
      expect(hoisted.auditLogRows[1].metadataAfter).toEqual({ scope: 'all', revokedCount: 3 });
    });
  });
