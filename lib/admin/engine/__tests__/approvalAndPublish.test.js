...
import { ENTITY_TYPE, VERSION_STATUS, REVISION_CONFLICT_ERROR_CODE } from '../../constants/enums';
...
describe.each(ADAPTER_CASES)('approvalService + publishService — %s', (label, makeAdapter, fields) => {
  ...
  it('reject() requires a rejectionNote', async () => {
    ...
    await expect(
      approvalService.reject(adapter, { parentId: parent.id, versionId: proposed.id, actorId: 'owner-1' })
  ...
  it('publish() only accepts a PROPOSED version — a DRAFT cannot be published directly', async () => {
    ...
    await expect(
      publishService.publish(adapter, { parentId: parent.id, versionId: draft.id, actorId: 'owner-1' })
