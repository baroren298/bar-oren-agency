...
// Established convention (see proposalLifecycle.test.js's header comment):
// `eventRepository` is the only thing standing between eventService and a
// real Prisma/Postgres write, so it's mocked with an in-memory array here
// too — this file's seedPublishedTalent()/startEditing() helpers drive
// proposalService.create()/submit() and approvalService.approve(), each of
// which calls eventService.emit(), so without this mock every test in this
// file would hit the real (test-env-null) prisma client during setup.
...
// reachable from this file (same reasoning as proposalLifecycle.test.js).
