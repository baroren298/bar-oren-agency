...
export function createInMemoryVersionStore() {
  ...
  return {
    ...
    /**
     * "Editable PROPOSED" sprint — mirrors
     * talentRepository.updateTalentVersionFields: merges `fields` into the
     * existing version's fields in place, touches no other column
     * (status, revisionNumber, etc. are untouched here — status-eligibility
     * is proposalService.update()'s job, not the store's).
     */
    async updateVersionFields(versionId, fields) {
      const version = versions.get(versionId);
      if (!version) throw new Error(`[inMemoryVersionStore] no version "${versionId}".`);
    ...
    /**
     * Cancel Editing / Discard Draft sprint — mirrors
     * talentRepository.deleteTalentVersion: a real row delete, not a status
     * flip. No status check here (same split as updateVersionStatus/
     * setVersionRejection above) — that's proposalService.discard()'s job.
     */
    async deleteVersion(versionId) {
      const existed = versions.delete(versionId);
      if (!existed) {
        throw new Error(`[inMemoryVersionStore] no version "${versionId}".`);
      }
    },
