...
const EVENT_TYPE_TO_ACTION_TYPE = Object.freeze({
  ...
  // lib/admin/talentArchiveService.js. ARCHIVED already exists (used since
  // Sprint 7B for clients/brands); RESTORED has existed in ActionType since
  // Section 4.1 but this is its first real emitter — no schema migration
  // needed for either. entityType is TALENT, entityId the talent id.
  [EVENT_TYPE.TALENT_ARCHIVED]: ACTION_TYPE.ARCHIVED,
  [EVENT_TYPE.TALENT_RESTORED]: ACTION_TYPE.RESTORED,
...
const ACTOR_FIELD_BY_ACTION_TYPE = Object.freeze({
  ...
  // Talent Archive & Restore feature — RESTORED has no dedicated actor FK
  // either (see header note). Reuses `updatedById`: same reasoning as
  // ACTIVATED/DEACTIVATED above — the acting Owner is the one who changed
  // the entity's lifecycle state back, not the original author/approver.
  [ACTION_TYPE.RESTORED]: 'updatedById',
});
