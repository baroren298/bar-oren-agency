...
export const he = {
  ...
  // CMS Error & Loading Boundaries sprint — copy for app/admin/error.jsx,
  // the standalone fallback shown when a render error escapes any admin
  // page. Deliberately generic (no page-specific wording) since one
  // boundary covers every /admin/** route. Never paired with error.message/
  // error.stack/error.digest anywhere — see that file's header comment for
  // why raw error detail is never shown to the person using this panel.
  error: {
    title: "משהו השתבש",
    body: "אירעה שגיאה בטעינת העמוד. אפשר לנסות שוב, ואם זה חוזר לקרות, לפנות לתמיכה.",
    retry: "נסה שוב",
    back: "חזרה ללוח הבקרה",
  },

  // Same sprint — visually-hidden label for app/admin/loading.jsx's
  // role="status" region. Kept alongside `error` above since both are
  // route-boundary copy, not page-specific copy.
  loading: {
    label: "טוען…",
  },
  ...
  social: {
    ...
    actions: {
      openLink: "פתח קישור",
      copyLink: "העתק קישור",
      // Social Remove sprint — mirrors he.gallery.actions.remove.
      remove: "הסר",
    },
    // Social Remove sprint — mirrors he.gallery.removeHint exactly: the
    // account will stop showing in the proposed list immediately, but if
    // it's already published, the removal only takes effect on the public
    // site after the Owner approves/publishes it.
    removeHint: "החשבון יוסר מהרשימה. אם הוא כבר מפורסם באתר, ההסרה תיכנס לתוקף רק לאחר אישור הבעלים.",
    platforms: {
      instagram: "אינסטגרם",
      tiktok: "טיקטוק",
      youtube: "יוטיוב",
  ...
  talent: {
    list: {
      ...
      filters: {
        ...
        draft: "טיוטות",
        // Talent Archive & Restore feature — a fifth pill, deliberately NOT
        // counted in "הכל" (unlike Hidden, which overlaps Published):
        // archived talents are removed from every other pill/view by
        // default (matching clients/brands' "ACTIVE only by default"
        // convention — see clientService.listClients) and only appear here.
        archived: "בארכיון",
      },
      ...
      hiddenBadge: "מוסתר",
      // Talent Archive & Restore feature — muted badge shown beside an
      // Archived talent's card, same role hiddenBadge plays for Hidden.
    ...
    // Talent Archive & Restore feature — server-side error copy for
    // talentArchiveService / the archive+restore API routes. Same
    // structure/tone as `create.errors` above; kept as its own group
    // (not nested under `detail`) since these strings are shared by the
    // service layer regardless of which UI surface triggered the call.
    archive: {
      errors: {
        notAuthenticated: "ההתחברות פגה. יש להתחבר מחדש.",
        ownerOnly: "רק לבעלים יש הרשאה להעביר מיוצג לארכיון או לשחזר אותו.",
        invalidBody: "הנתונים שנשלחו לא תקינים.",
        talentNotFound: "המיוצג לא נמצא.",
        talentAlreadyArchived: "המיוצג כבר נמצא בארכיון.",
        talentNotArchived: "המיוצג אינו בארכיון.",
        talentArchivedReadOnly: "המיוצג נמצא בארכיון ולא ניתן לערוך אותו. יש לשחזר אותו קודם.",
    ...
    detail: {
      ...
      // Talent Archive & Restore feature — copy for the header's
      // Archive/Restore action (TalentArchiveAction.jsx) and its
      // confirmation dialog. Deliberately separate from `visibilityAction`
      // above: archive is an entity-lifecycle removal (OWNER only, takes
      // effect immediately — no Draft/Submit/Approve step), not another
      // content-visibility toggle, so it gets its own distinct action,
      // wording, and badge rather than reusing that button/flow.
      archiveAction: {
        archiveLabel: "העבר לארכיון",
        restoreLabel: "שחזר מהארכיון",
        archiveLoading: "מעביר לארכיון…",
        restoreLoading: "משחזר…",
        genericError: "משהו נכשל. נסה שוב.",
        confirmArchiveTitle: "להעביר את המיוצג לארכיון?",
        confirmArchiveBody:
          "המיוצג יוסר מיד מהאתר הציבורי ומרשימת המיוצגים הפעילים. שום מידע לא יימחק — כל ההיסטוריה, התמונות, הרשתות החברתיות ונתוני ה-SEO נשמרים במלואם, וניתן לשחזר את המיוצג בכל שלב.",
        confirmRestoreTitle: "לשחזר את המיוצג מהארכיון?",
        confirmRestoreBody:
          "המיוצג יחזור מיד לרשימת המיוצגים הפעילים ולאתר הציבורי, בדיוק כפי שהיה לפני ההעברה לארכיון — לא יידרש פרסום מחדש.",
        confirmArchiveCta: "העבר לארכיון",
        confirmRestoreCta: "שחזר",
        confirmCancel: "ביטול",
        archivedBadge: "בארכיון",
        // Shown as a banner on an archived talent's workspace page — this
        // talent is read-only until restored (propose/submit/approve are
        // all blocked server-side; see talent-archive-guard.js).
        archivedNoticeTitle: "המיוצג נמצא בארכיון",
        archivedNoticeBody:
          "המיוצג מוסתר מהאתר הציבורי ואינו ניתן לעריכה. יש לשחזר אותו כדי להמשיך לערוך או לפרסם שינויים.",
      },
