...
export const he = {
  nav: {
    ...
    // when role === ROLE.OWNER (see that file's header comment).
    users: "משתמשים",
  },
  ...
  users: {
    ...
    detail: {
      ...
      profile: {
        description: "שם התצוגה מוצג בכל מקום במסך הניהול. האימייל הוא מזהה ההתחברות ואינו ניתן לעריכה כאן.",
      },
      role: {
        description: "תפקיד קובע אילו פעולות ניהול זמינות למשתמש.",
        // Role editing is deferred — see this key's explanation, shown
        // directly in the UI rather than only in a code comment, per this
        // sprint's explicit "explain why" requirement. Reasoning: safely
        // changing role requires the same guardrails setActive already has
        // for deactivation (never demote the last active Owner, and decide
        // whether an Owner may demote themselves) — building and testing
        // that correctly is bigger than this sprint's scope, so role stays
        // read-only for now rather than shipping a half-safe toggle.
        readOnlyNote:
          "עריכת תפקידים תתווסף בעדכון עתידי. שינוי תפקיד מחייב את אותם מנגנוני הגנה שקיימים כיום להשבתת משתמש (למשל, לא ניתן להסיר את הבעלים הפעיל האחרון), ובנייתם ובדיקתם המלאה חורגת מהיקף הספרינט הנוכחי — כדי לא לחשוף פעולה לא בטוחה, התפקיד מוצג כרגע לקריאה בלבד.",
      },
      security: {
        description: "איפוס סיסמה קובע סיסמה זמנית חדשה למשתמש זה. יש למסור אותה לו באופן מאובטח.",
        newPasswordLabel: "סיסמה זמנית חדשה",
        newPasswordPlaceholder: "לפחות 8 תווים",
        submit: "אפס סיסמה",
        submitting: "מאפס…",
        success: "הסיסמה אופסה בהצלחה. מסרו אותה למשתמש באופן מאובטח.",
      },
      status: {
