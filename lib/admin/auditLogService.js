 ...
 *      repository) can therefore never render. Sprint 3b adds two more
 *      named keys to the allowlist — `scope` / `revokedCount` (session
 *      revocation events) — same discipline, never a sid.
...
function buildSafeDetails(metadataAfter) {
  ...
  // UserSessionRevoked / UserSessionsRevoked payloads (Sprint 3b):
  // { scope: 'single'|'all' } / { scope: 'all', revokedCount: n }. Named
  // keys only, same allowlist discipline as the rest of this function —
  // never a sid, token, or any request/connection data (sessionManagement
  // Service never puts those in the payload in the first place).
  if (metadataAfter.scope === 'single' || metadataAfter.scope === 'all') {
    details.scope = metadataAfter.scope;
  }
  if (typeof metadataAfter.revokedCount === 'number' && Number.isFinite(metadataAfter.revokedCount)) {
    details.revokedCount = metadataAfter.revokedCount;
  }
