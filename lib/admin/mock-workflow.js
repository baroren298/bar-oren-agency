...
//
// Pre-merge stabilization: the default changed from MOCK_WORKFLOW_ITEMS to
// [] so no caller can accidentally render the hardcoded demo items in
// production. Pass MOCK_WORKFLOW_ITEMS (still exported below, unchanged)
// explicitly if demo data is ever needed for local development.
export function getWorkflowSections(items = []) {
