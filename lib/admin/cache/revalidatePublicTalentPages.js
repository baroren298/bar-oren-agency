 ...
 *
 * Talent Visibility — Issue 1 fix: also revalidates the homepage
 * (`/[locale]`), which renders its own Featured Talent section via
 * `getPublicFeaturedTalent()` (lib/public/talent.js) — the same
 * visibility-filtered source as the routes below. Without this, hiding or
 * restoring a featured talent would still be invisible on the homepage
 * until its own ISR window (`app/[locale]/page.jsx` has no explicit
 * `revalidate` export, so it follows Next's default) happened to expire.
 */
