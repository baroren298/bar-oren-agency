...
export function buildGalleryImages(galleryImages, displayName) {
  return (galleryImages || []).map((row, index) => ({
    ...
    // Gallery Image Removal sprint — carried through so the client can
    // both render the current removal state (MediaGalleryEditor filters a
    // HIDDEN row out of what it displays) and round-trip it back on the
    // next Save Draft. Defaults to ACTIVE for any row shape that predates
    // this field (a raw repository row is expected to always have it, but
    // this mirrors every other field's defensive `?? null`-style fallback
    // above).
