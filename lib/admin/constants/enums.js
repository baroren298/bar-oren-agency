 ...
 *
 * SESSION_REVOKED / SESSIONS_REVOKED — Sprint 3b (Session Management API):
 * Owner-initiated revocation of one / all of a target user's sessions.
 * Same honest-narrative rationale, same additive-migration pattern
...
export const ENTITY_TYPE = Object.freeze({
  ...
  USER: 'USER',
  // Sprint 7B (Clients & Brands Foundation) — target discriminators for
  // client/brand events and audit rows. Additive migration
  // 20260717120000_add_clients_and_brands.
  CLIENT: 'CLIENT',
  BRAND: 'BRAND',
});
...
/**
 * Sprint 7B (Clients & Brands Foundation) — error codes stamped onto the
 * error clientRepository/brandRepository's create/update transactions
 * throw when a normalized name collides (Client globally; Brand within its
 * Client — archived rows included, per the confirmed product rule). Same
 * layering rationale as the slug codes above: the repository throws them,
 * clientService recognizes them and translates to friendly Hebrew 409s,
 * and neither layer imports the other for a constant.
 */
export const CLIENT_NAME_CONFLICT_ERROR_CODE = 'CLIENT_NAME_CONFLICT';
export const BRAND_NAME_CONFLICT_ERROR_CODE = 'BRAND_NAME_CONFLICT';
