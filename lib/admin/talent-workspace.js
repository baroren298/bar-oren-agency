 ...
 * @returns {boolean} false whenever either argument is missing — "unknown"
 *   must never be treated as "safe to discard"
 */
export function talentVersionIsUnchangedFromPublished(pendingVersion, publishedVersion) {
  ...
  return Object.keys(pendingFields).every(
    (key) => normalize(pendingFields[key]) === normalize(publishedFields[key])
  );
}
