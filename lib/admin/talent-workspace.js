...
import { VERSION_STATUS, TALENT_VISIBILITY } from './constants/enums';
...
/**
 * Admin Talent List single-badge sprint — picks the one workflow/visibility
 * badge a list card (TalentQueueRow.jsx) should show, instead of rendering
 * the workflow badge and a separate "מוסתר" visibility badge side by side
 * (the detail page still shows both independently — see
 * deriveDetailWorkflowStatus + talentVisibilityLabel, both untouched).
 *
 * No new business logic: this only re-reads the exact same two derivations
 * the list already computes (deriveListWorkflowStatus, isListTalentHidden)
 * and decides which single label/tone pair wins, per the product rule
 ...
 *
 *   - not yet published (draft / waiting for approval / — if the list's
 *     data shape is ever widened — changes requested): show that workflow
 *     status, unchanged.
