...
export const talentRepository = {
   ...
   * @param {object} params.fields - { platform, label, customLabel, handle, url, sortOrder }
  ...
  async updateTalentSocialFields(socialId, fields) {
    ...
    const WRITABLE_COLUMNS = ['platform', 'label', 'customLabel', 'handle', 'url', 'sortOrder'];
   ...
   */
  async submitDraftSocialsForTalent(talentId) {
    if (!talentId) {
    ...
    return prisma.$transaction(async (tx) => {
      const drafts = await tx.talentSocial.findMany({
        where: { talentId, versionStatus: VERSION_STATUS.DRAFT },
        select: { id: true },
      });
   ...
   * getProposedSocialsForTalent exactly, including the same
   * forward-compatibility note: no write path here sets lifecycleStatus on
   * a TalentGalleryImage row (updateTalentGalleryImageFields's
   * WRITABLE_COLUMNS below excludes it), so in practice every row this
   * returns is ACTIVE.
   *
   * @param {string} talentId
   * @returns {Promise<Array<object>>}
  ...
  async updateTalentGalleryImageFields(imageId, fields) {
    ...
    const WRITABLE_COLUMNS = ['order', 'altHe', 'altEn', 'position', 'scale', 'mobileOrder'];
   ...
   */
  async submitDraftGalleryImagesForTalent(talentId) {
    if (!talentId) {
