...
export const talentRepository = {
   ...
   *
   * Per this sprint's product decision ("Owner-created talent can be
   * created directly as the initial editable/published admin record" —
   * only OWNER exists at launch, Section 11), the first version is written
   * straight as PUBLISHED rather than entering the normal Draft -> Proposed
   * -> Approve -> Publish cycle: there is no prior published version to
   * protect, no concurrent editor to conflict with, and nothing yet to
   * review against. This is the one talent-creation-specific exception to
   * "publishService.publish() is the only path to PUBLISHED" (Section
   * 13.5) — it applies only to a talent's first version, at creation time;
   * every subsequent edit to this talent still goes through the unchanged
   * proposal/approval/publish flow.
   *
   * Mirrors `publishTalentVersion`'s shape: one `prisma.$transaction`
   * covering the Talent insert, the TalentVersion insert, and the
   * currentPublishedVersionId repoint, so a half-created talent (parent row
   * with no version, or vice versa) can never be observed.
   *
   ...
   * @param {object} params.fields - TalentVersion business fields (name, bioHe, ...)
  ...
  async createTalentWithInitialVersion({ slug, fields, createdById }) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return { talent: publishedTalent, version };
    });
  },
