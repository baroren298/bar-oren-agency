...
/**
 * Save Draft birthDate regression fix — TalentVersion.birthDate is a
 * `DateTime?` column, but every UI caller works with the plain date-only
 * 'YYYY-MM-DD' string convention already established by ComparisonView's
 * "date" field type (components/admin/ComparisonView.jsx's ProposedField,
 * `<input type="date">`) and mirrored read-side by lib/public/talent.js's
 * `new Date(version.birthDate).toISOString().slice(0, 10)`. No repository
 * write site previously normalized this — insertTalentVersion,
 * createTalentWithInitialVersion, and updateTalentVersionFields all just
 * forwarded `fields.birthDate` to Prisma as-is. That was invisible as long
 * as every caller either sent `null`/`undefined` or an already-persisted
 * Date instance (e.g. extractTalentVersionFields seeding a new Draft from
 * a Published version's own `birthDate` Date), but broke the moment a
 * caller sent a fresh 'YYYY-MM-DD' string — exactly what the unified
 * Details Save Draft path started doing once Profile Image/birthDate moved
 * into the single combined payload: Prisma's DateTime scalar rejects a
 * bare date-only string with "Invalid value for argument birthDate:
 * premature end of input. Expected ISO-8601 DateTime."
 *
 * This is the one, single normalization used by every TalentVersion
 * write site that accepts `birthDate` (no second date convention):
 *   - `null`/`undefined`/an already-`Date` value pass through untouched —
 *     nothing to normalize, and an absent key is never even passed here
 *     (updateTalentVersionFields's own hasOwnProperty allowlist loop keeps
 *     that field out of `data` entirely, preserving "absent = untouched").
 *   - a 'YYYY-MM-DD' string becomes `new Date('YYYY-MM-DD')`. Per the
 *     ISO-8601/JS Date spec, a date-only string with no time component is
 *     always parsed as UTC midnight — never as local midnight — in every
 *     server timezone, so the calendar date this produces can never shift
 *     by a day in either direction. (The unsafe alternative this
 *     deliberately avoids is constructing a Date from separate y/m/d
 *     components, e.g. `new Date(y, m, d)`, which uses local time and can
 *     shift the date across a UTC day boundary depending on server TZ.)
 *
 * @param {string|Date|null|undefined} value
 * @returns {Date|null|undefined}
 */
function normalizeBirthDateForPrisma(value) {
...
/**
 * Talent Published Sort Order sprint — apply publish-time automatic
 * reordering, inside `publishTalentVersion`'s transaction.
 *
 * Kept as a module-level helper rather than inlined so `publishTalentVersion`
 * still reads as the short sequence of gates and writes it always was, and
 * so the ordering concern has one obvious home. All the arithmetic lives in
 * lib/admin/published-order.js (pure, DB-free, separately tested); this
 * function is only the I/O around it: take the lock, read the roster, apply
 * the diff.
 *
 * THE NORMALIZATION GATE. This deliberately does nothing at all unless the
 * current published roster is already canonical (contiguous 1..N, no nulls,
 * no duplicates). The seeded production data is 0-based and may contain
 * duplicates — so the very first publish after this ships would otherwise
 * renumber every talent in the roster as an invisible side effect of
 * publishing one of them. That renumbering is a deliberate, reviewable
 * data-normalization decision, not something a routine publish should
 * perform on an operator's behalf. Until it has run, publishes behave
 * exactly as they did before this sprint, and the skip is reported (both a
 * server-log warning and `ordering.applied: false` on the result) rather
 * than being silent.
 *
 * @param {object} tx - the interactive Prisma transaction client
 * @param {object} params
 * @param {object} params.talent - the parent Talent row
 * @param {object} params.version - the TalentVersion row being published
 * @param {string} params.versionId
 * @returns {Promise<{ versionData: object, result: object }>} `versionData`
 *   is spread into the publish update (empty when skipped); `result` is the
 *   caller-visible outcome metadata.
 */
async function applyPublishedOrdering(tx, { talent, version, versionId }) {
  // The lock is taken BEFORE the roster read, and released by Postgres on
  // COMMIT/ROLLBACK. See TALENT_PUBLISHED_ORDER_LOCK_KEY's comment below for
  // why a read-modify-write over the roster needs one at all.
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(${TALENT_PUBLISHED_ORDER_LOCK_KEY}::bigint)`;

  // The ordering scope: talents that actually hold a position in the public
  // roster. ARCHIVED/soft-deleted talents are excluded (they are gone from
  // the public site, and letting them hold slots would leave permanent
  // phantom gaps). Talents whose published version is HIDDEN are
  // deliberately INCLUDED — hiding is a rendering decision, and a hidden
  // talent that keeps its slot comes back where it was instead of
  // colliding with whoever took its number.
  const publishedTalents = await tx.talent.findMany({
    where: {
    ...
    select: {
      ...
      currentPublishedVersion: { select: { sortOrder: true } },
    },
  });

  const currentOrder = publishedTalents.map((row) => ({
    talentId: row.id,
    versionId: row.currentPublishedVersionId,
    sortOrder: row.currentPublishedVersion ? row.currentPublishedVersion.sortOrder : null,
  }));

  if (!isCanonicalPublishedOrder(currentOrder)) {
    console.warn(
      '[talentRepository.publishTalentVersion] published talent ordering is not normalized ' +
        `(${currentOrder.length} published talents; expected the contiguous positions ` +
        '1..N with no nulls or duplicates) — publish-time reordering SKIPPED for this ' +
        'publish. Run the one-off ordering normalization to enable it.'
    );
    return {
      versionData: {},
      result: {
  ...
  const self = currentOrder.find((entry) => entry.talentId === talent.id) || null;

  // A first publish grows the roster by one; a re-publish does not.
  const targetLength = self ? currentOrder.length : currentOrder.length + 1;

  // The anti-drift input: what the draft's base version said the position
  // was. Compared against the draft's own value to tell "the editor moved
  // this talent" apart from "the draft merely inherited a number that has
  // since been shifted by somebody else's publish". See
  // resolvePublishPosition's doc comment.
  let basedOnSortOrder = null;
  if (version.basedOnVersionId) {
    const basedOnVersion = await tx.talentVersion.findUnique({
      where: { id: version.basedOnVersionId },
      select: { sortOrder: true },
    });
    basedOnSortOrder = basedOnVersion ? basedOnVersion.sortOrder : null;
  }

  const resolved = resolvePublishPosition({
    requestedSortOrder: version.sortOrder,
    currentPosition: self ? self.sortOrder : null,
    basedOnSortOrder,
    targetLength,
  });

  const { changes } = computePublishedOrder({
    currentOrder,
    movingTalentId: talent.id,
    movingVersionId: versionId,
    position: resolved.position,
  });

  // Shift every other affected talent. Two deliberate properties here:
  //
  //   1. Only `sortOrder` is written. These talents' PUBLISHED version rows
  //      are being edited in place, which is otherwise unheard of in this
  //      schema — justified because a roster position is list metadata, not
  //      profile content, and creating a fresh published version per
  //      shifted talent would be far worse.
  //   2. Talent.revisionNumber is deliberately NOT incremented for them.
  //      Their content did not change, and bumping it would turn every open
  //      Draft on those talents into a spurious REVISION_CONFLICT the next
  //      time its author tried to publish.
  //
  // The moving talent is skipped: its position rides along on the publish
  // update itself, via the returned `versionData`.
  let shifted = 0;
  for (const change of changes) {
    if (change.talentId === talent.id) continue;
    await tx.talentVersion.update({
      where: { id: change.versionId },
  ...
  return {
    versionData: { sortOrder: resolved.position },
    result: {
      applied: true,
...
/*
 * Talent Published Sort Order sprint — the Postgres advisory-lock key that
 * serializes publish-time reordering.
 *
 * Why a lock at all: the reorder is a read-modify-write over the whole
 * published roster (read every published talent's position, compute the new
 * ordering, write the affected band). Postgres defaults to READ COMMITTED,
 * so two publishes running concurrently would each read the same snapshot,
 * each compute a valid-looking ordering against it, and then both commit —
 * recreating exactly the duplicate positions this sprint exists to
 * eliminate. Transaction isolation alone does not prevent that; the
 * conflicting rows are ones neither transaction read as its own.
 *
 * Why an ADVISORY lock specifically: the rows that must not change under us
 * are "every talent's current published version", which is a join across
 * two tables, not a row set Prisma's query API can express a `SELECT ...
 * FOR UPDATE` over. A transaction-scoped advisory lock on one fixed key is
 * the smallest correct alternative — it serializes publishes against each
 * other and nothing else, and Postgres releases it automatically on COMMIT
 * or ROLLBACK, so a failed publish can never strand it.
 *
 * The value itself is arbitrary but must stay stable: changing it would
 * mean an old and a new deployment no longer exclude one another.
 */
const TALENT_PUBLISHED_ORDER_LOCK_KEY = 8143276105;

/**
 * Talent Published Sort Order sprint — read the current published roster
 * (id, current published version id, its sortOrder), shared by the
 * archive/restore ordering helpers below and by `applyPublishedOrdering`
 * above. Same scope as that function: ACTIVE, not soft-deleted, currently
 * published. Deliberately not exported — internal to this file only.
 *
 * @param {object} tx
 * @returns {Promise<Array<{ talentId: string, versionId: string, sortOrder: number|null }>>}
 */
async function readPublishedOrder(tx) {
  const publishedTalents = await tx.talent.findMany({
    where: {
      status: LIFECYCLE_STATUS.ACTIVE,
      deletedAt: null,
  ...
  return publishedTalents.map((row) => ({
    talentId: row.id,
    versionId: row.currentPublishedVersionId,
    sortOrder: row.currentPublishedVersion ? row.currentPublishedVersion.sortOrder : null,
  }));
...
/**
 * Talent Published Sort Order sprint — close the gap archiving a talent
 * leaves behind. Runs inside archiveTalent's own transaction, under the
 * SAME advisory lock and the SAME normalization gate as
 * `applyPublishedOrdering` (the publish path) — see that function's
 * header comment for the full rationale, not repeated here.
 *
 * Called BEFORE the talent's own status flips to ARCHIVED, so the roster
 * read still sees it as ACTIVE; `computeOrderAfterRemoval` is what
 * excludes it from the result. Skips entirely (no writes) when the talent
 * has no published version, or when the roster is not yet canonical
 * 1..N — same "leave it exactly as it was" behavior as an un-normalized
 * publish.
 *
 * @param {object} tx
 * @param {string} talentId
 */
async function applyPublishedOrderingForArchive(tx, talentId) {
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(${TALENT_PUBLISHED_ORDER_LOCK_KEY}::bigint)`;

  const currentOrder = await readPublishedOrder(tx);
  const self = currentOrder.find((entry) => entry.talentId === talentId);
  if (!self) return; // not currently in the published roster — nothing to close

  if (!isCanonicalPublishedOrder(currentOrder)) {
    console.warn(
      '[talentRepository.archiveTalent] published talent ordering is not normalized — ' +
        'archive-time reordering SKIPPED for this archive.'
    );
    return;
  }

  const { changes } = computeOrderAfterRemoval({ currentOrder, removingTalentId: talentId });

  // Only the shifted talents' sortOrder is written — same "list metadata,
  // not content, no revisionNumber bump" rule as publish-time shifts.
  for (const change of changes) {
    await tx.talentVersion.update({ where: { id: change.versionId }, data: { sortOrder: change.sortOrder } });
  }
}

/**
 * Talent Published Sort Order sprint — append a restored talent to the end
 * of the live published roster, rather than letting it come back holding
 * whatever stale `sortOrder` it had before it was archived (which could
 * now collide with a live talent). Runs inside restoreTalent's own
 * transaction, under the SAME advisory lock and normalization gate as
 * archive/publish.
 *
 * Called BEFORE the talent's own status flips back to ACTIVE, so the
 * roster read naturally excludes it (it is still ARCHIVED at read time) —
 * `computePublishedOrder` is reused exactly as publish uses it, with the
 * position resolved to "one past the current end" (append).
 *
 * Skips entirely when the talent has no currentPublishedVersionId (never
 * published — nothing to order, and none is manufactured here) or the
 * roster is not yet canonical.
 *
 * @param {object} tx
 * @param {string} talentId
 */
async function applyPublishedOrderingForRestore(tx, talentId) {
  await tx.$executeRaw`SELECT pg_advisory_xact_lock(${TALENT_PUBLISHED_ORDER_LOCK_KEY}::bigint)`;
  ...
  if (!target || !target.currentPublishedVersionId) return;

  const currentOrder = await readPublishedOrder(tx);

  if (!isCanonicalPublishedOrder(currentOrder)) {
    console.warn(
      '[talentRepository.restoreTalent] published talent ordering is not normalized — ' +
        'restore-time reordering SKIPPED for this restore.'
    );
    return;
  }
  ...
  const { changes } = computePublishedOrder({
    currentOrder,
    movingTalentId: talentId,
    movingVersionId: target.currentPublishedVersionId,
    position: currentOrder.length + PUBLISHED_ORDER_BASE, // always append
  });
...
export const talentRepository = {
  ...
  async archiveTalent(talentId, archivedByUserId) {
    ...
    // Talent Published Sort Order sprint — archiving must close the gap it
    // leaves in the published roster, atomically with the status flip. See
    // applyPublishedOrderingForArchive's header comment.
    return prisma.$transaction(async (tx) => {
      await applyPublishedOrderingForArchive(tx, talentId);
  ...
  async restoreTalent(talentId) {
    ...
    // Talent Published Sort Order sprint — restoring must append (never
    // reuse a stale historical position), atomically with the status flip.
    // See applyPublishedOrderingForRestore's header comment.
    return prisma.$transaction(async (tx) => {
      await applyPublishedOrderingForRestore(tx, talentId);
  ...
  async listTalentVersionsForTalent(talentId) {
    return prisma.talentVersion.findMany({
      ...
      include: {
        ...
        // Talent Details Lifecycle Unification sprint — the pending
        // DRAFT/PROPOSED version the admin talent detail page reads comes
        // from this list query (versionService.getCurrentDraftOrProposed →
        // listVersionsForParent), so the profile image relation must ride
        // along here too, exactly like podcastImageAsset below already
        // does — otherwise a Draft's own profile image (including one set
        // at Talent creation, before any Published version exists) never
        // resolves a blobUrl and the Details tab's proposed image silently
        // renders empty (or falls back to the Published image, if any).
        // Same additive-include reasoning as getTalentVersionById above;
        // blobUrl only, still a pure read.
        profileImageAsset: { select: { blobUrl: true } },
  ...
  async insertTalentVersion({
    ...
    return prisma.talentVersion.create({
      data: {
        ...
        birthDate: normalizeBirthDateForPrisma(fields.birthDate),
        bioHe: fields.bioHe,
        bioEn: fields.bioEn,
  ...
  async updateTalentVersionFields(versionId, fields) {
    ...
    for (const key of WRITABLE_COLUMNS) {
      if (Object.prototype.hasOwnProperty.call(fields, key)) {
        // Save Draft birthDate regression fix — the one write site that
        // accepts a fresh 'YYYY-MM-DD' string from the UI rather than an
        // already-persisted Date; see normalizeBirthDateForPrisma's header
        // comment above for why this is the single shared normalization,
        // not a second convention. Every other column is untouched.
        data[key] = key === 'birthDate' ? normalizeBirthDateForPrisma(fields[key]) : fields[key];
      }
    }
  ...
  async publishTalentVersion(versionId, { expectedRevisionNumber, approvedById } = {}) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      // ─── Talent Published Sort Order sprint ────────────────────────────
      // Publish-time automatic reordering. This is the ONLY place in the
      // system permitted to change the effective published ordering:
      // Save Draft (updateTalentVersionFields) and Submit
      // (updateTalentVersionStatus) deliberately never reach this code, so
      // a Draft may freely hold a position that conflicts with a live
      // talent without disturbing the public roster until it is actually
      // published. That isolation is structural — the public site reads
      // only Talent.currentPublishedVersion — not something enforced here.
      //
      // Everything below runs inside the transaction that already wraps the
      // supersede/publish/repoint writes, so a failure anywhere (including
      // the slug gates above) rolls the reorder back with it. There is no
      // state in which the roster is half-shifted or a talent is published
      // without its position applied.
      const ordering = await applyPublishedOrdering(tx, { talent, version, versionId });
      ...
      const publishedVersion = await tx.talentVersion.update({
        ...
        data: {
          ...
          approvedById,
          approvedAt: new Date(),
          // Folded into the publish write rather than issued as a second
          // update: the row being published must carry its authoritative
          // position, and the number copied onto it from the draft may be
          // stale (see published-order.js's anti-drift note). Spreads to
          // nothing when reordering was skipped, leaving this update
          // byte-for-byte what it was before this sprint.
          ...ordering.versionData,
        },
      });
      ...
      // `ordering` is additive metadata (applied/reason/position/shifted).
      // Existing callers destructure { version, parent } and are unaffected;
      // it exists so a publish that silently skipped reordering because the
      // roster is not normalized yet is observable rather than invisible.
      return { version: publishedVersion, parent: publishedTalent, ordering: ordering.result };
    });
  },
