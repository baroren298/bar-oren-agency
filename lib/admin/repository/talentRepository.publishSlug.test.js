...
// Talent Published Sort Order sprint — publishTalentVersion's transaction
// now also takes an advisory lock and reads the published roster before it
// writes (see applyPublishedOrdering). Both are stubbed here so these slug
// tests keep exercising only the slug gates: an empty roster means the
// reorder resolves to "first published talent, position 1" and shifts
// nobody, leaving every assertion below about Talent.slug untouched.
