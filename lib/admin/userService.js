...
export const userService = {
  ...
  async setActive(userId, isActive, { actorId, actorRole, correlationId, requestMetadata } = {}) {
    ...
    // Sprint 3a (Session Security Foundation): deactivation must kill the
    // user's live sessions in the SAME transaction as the isActive flip
    // (commit together or roll back together — a deactivated user must
    // fail on their very next request, never ride out an old token).
    // Reactivation uses the plain update and revokes nothing: it must not
    // resurrect previously revoked sessions (revokedAt is set-once).
    const updated = nextActive
      ? await userRepository.setActive(userId, true)
      : await userRepository.setActiveAndRevokeSessions(userId, false);
  ...
  async resetPassword(userId, temporaryPassword, { actorId, actorRole, correlationId, requestMetadata } = {}) {
    ...
    const passwordHash = await hashPassword(password);
    // Sprint 3a (Session Security Foundation): the hash overwrite and the
    // revocation of every live session ride ONE transaction — a reset can
    // never commit while the old credential's sessions survive. Applies to
    // an Owner self-reset too (their own sessions die; next request lands
    // on /admin/login) — intended and security-correct.
    const updated = await userRepository.updatePasswordHashAndRevokeSessions(userId, passwordHash);
