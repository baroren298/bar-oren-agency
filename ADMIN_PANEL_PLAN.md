...
**Implementation status note (Sprint 3.6):** `proposalService`, `conflictService`, `approvalService`, `publishService`, `versionService`, `eventService`/`eventTypes`, `adapterContract`, and `talentAdapter` are implemented (Sprints 3.2–3.5). Sprint 3.6 added a second adapter — `createEntityAdapter` (`lib/admin/engine/adapters/entityAdapter.js`), wired against new `entityRepository` primitives — proving the same, unmodified engine services run against a second entity type. `siteContentAdapter`, `seoAdapter`, `legalPageAdapter`, route handlers/`/admin` UI, and `mapToPublicShape`/Live Preview remain unbuilt (Section 13.14 sub-phases 7–8 and Phase 4 onward).

**Implementation status note (Sprint 3.7) — correction to the Sprint 3.6 note above:** Sprint 3.6's note incorrectly stated Phase 3 success criteria 1–7 were already met. In fact, `auditLogListener` was still Sprint 3.2's throwing placeholder, unregistered in `listeners/index.js`, and `auditLogRepository.record()` was still a `notImplemented()` stub — so no AuditLog row had ever actually been written from the Event stream, meaning criterion #6 (Section 13.17: "AuditLog rows are generated from the Event stream by `auditLogListener`, never written directly by any other code path") was not met. Sprint 3.7 fixes this: `auditLogRepository.record()` now does a real `prisma.auditLog.create()`, `auditLogListener.js` now does a real `EVENT_TYPE → ActionType` projection (`PROPOSAL_CREATED→CREATED`, `PROPOSAL_SUBMITTED→PROPOSED`, `PROPOSAL_APPROVED→APPROVED`, `PROPOSAL_REJECTED→REJECTED`, `PROPOSAL_UPDATED→UPDATED`), and it is now registered live in `listeners/index.js`. Criterion #6 is now met.

**Known, deliberate gap carried forward from Sprint 3.7:** `VERSION_PUBLISHED` events are not projected to AuditLog. `ActionType` (prisma/schema.prisma) has no `PUBLISHED` value, and adding one is a schema migration — out of scope for this sprint and not performed. `auditLogListener` treats `VERSION_PUBLISHED` (and any other event type with no entry in its mapping table) as a recognized no-op, not an error. In v1's composition (`approvalService.approve()` always calls `publishService.publish()` in the same transaction), the `PROPOSAL_APPROVED` audit row already records that the action happened, so this is a soft gap today — but it will need real resolution (adding `ActionType.PUBLISHED` via a migration) before "approve now, publish later" or scheduled publishing (Section 13.5) are built, since those flows can publish without a fresh `approve()` call to piggyback on. Also still deferred: `auditService.js` (read-side queries over the Event stream for `/admin/history`), `ipAddress`/`userAgent` population (no request context exists yet — no routes), and retry/backfill tooling for listener failures.
...
2. **Auth/security** *(done)* — Owner login, session handling, `/admin/*` and `/api/admin/*` gating in `proxy.js`, environment-variable wiring for secrets (Section 11). Done before any real admin UI is built, not after, since every later phase assumes the admin is already access-controlled.
...
Every adapter implements: a static `entityType` (one of the `EntityType` enum values), `getParent(id)`, `getVersion(versionId)`, `listVersionsForParent(parentId)`, `insertProposedVersion(fields, meta)`, `submitVersion(versionId)`, `validate(fields)`, `mapToPublicShape(version, related)` (used by Live Preview, Section 7 — only meaningful where `capabilities.supportsPreview` is true), and the static `capabilities` object (13.4).

**`entityType`** — each adapter exposes its own fixed entity type as a static property. This is what lets generic engine services (`proposalService`, `conflictService`, `eventService`) emit `Event` rows carrying the correct `entityType` (Section 13.3.1) without ever branching on which adapter is in play — the engine asks the adapter "what are you," it never guesses or maintains its own adapter-to-entityType lookup. This preserves the locked architecture exactly as designed: the engine stays generic (Section 13.9), and translation/metadata about a specific entity stays confined to that entity's adapter (Section 13.16, "Adapters own translation only").

**`submitVersion(versionId)`** — each adapter exposes one narrow method for the DRAFT → PROPOSED transition described in Section 13.3 (`proposalService.submit()`). This must stay narrow and must not be replaced or generalized into a broader `updateVersionStatus(versionId, status)`-style method — a general status setter on every adapter would make it possible for some future caller to set a version straight to `PUBLISHED` from outside `publishService`, which is exactly what Section 13.5's "`publishService.publish()` remains the only code path that ever sets `PUBLISHED`" and Section 13.18's "no bypassing the Proposal/Approval flow" guardrail forbid. `submitVersion` performs the DRAFT → PROPOSED transition only and must never be capable of publishing.

Neither addition is a new architectural decision — both are documentation alignment with requirements the locked architecture already implies elsewhere: `Event.entityType` being a required, non-nullable column (Section 13.3.1); engine services being required to stay entity-agnostic (Section 13.9); the strict, no-skipping layer boundaries of Section 13.15; and the no-bypass guardrail on the Proposal/Approval/Publish flow (Section 13.18). Section 13.10's original list simply predated the DRAFT/submit split introduced earlier in Section 13.3 and didn't yet name the method or metadata field that split requires — this entry closes that gap without changing what any other part of the contract does.
