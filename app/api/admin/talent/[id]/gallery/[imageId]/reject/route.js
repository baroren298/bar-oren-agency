/*
 * POST /api/admin/talent/[id]/gallery/[imageId]/reject — Gallery Sprint 1.
 *
 * Sibling to app/api/admin/talent/[id]/socials/[socialId]/reject/route.js
 * — Owner-only (requireOwner), identical shape, requires a non-empty
 * `rejectionNote` in the body.
 *
 * Behavior:
...
export async function POST(request, { params }) {
  ...
  if (!rejectionNote) {
    return NextResponse.json(
      { error: he.gallery.errors.rejectionNoteRequired, code: 'REJECTION_NOTE_REQUIRED' },
      { status: 400 }
    );
  }
  ...
  try {
    const { image } = await galleryService.reject(talentAdapter, {
      parentId: id,
      imageId,
    ...
    console.error('[POST /api/admin/talent/[id]/gallery/[imageId]/reject] failed to reject:', error);
