/*
 * POST /api/admin/brands/[id]/archive — RETIRED route (Website CMS Focus
 * Cleanup).
 *
 * Retired from the Website CMS: returns the uniform retired 404 (see
 * ../../../retiredRouteContract.js) BEFORE the OWNER-only gate or the brand
...
vi.mock('@/lib/admin/clientService', () => ({
  clientService: { archiveBrand: mocks.archiveBrand },
}));
...
describe('POST /api/admin/brands/[id]/archive (retired)', () => {
  it('returns the retired 404 without touching the OWNER gate or the service', async () => {
    const req = makeRetiredRequest();
    await expectRetiredResponse(await POST(req, makeRetiredCtx('brand-1')), { req });
